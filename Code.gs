...
function saveAllDocuments_(docs) {
  ...
  try {
    ...
    if (!hasLock) {
      Logger.log('Could not acquire lock for saving documents');
      return;
    }
    var props = PropertiesService.getScriptProperties();
...
function saveAllContacts(contacts) {
  ...
  try {
    ...
    if (!hasLock) {
      Logger.log('Could not acquire lock for saving contacts');
...
function saveSignedDocument(signedBase64, docId, signerName, signerId) {
  var folder = getOrCreateFolder_();
  var docs = getAllDocuments();
  var doc = null;
  var docIdx = -1;
  ...
  // Update only the specific signer who signed (not all signers)
  var signers = doc.signers || [];
  var signerFound = false;
  for (var j = 0; j < signers.length; j++) {
    if (signerId && signers[j].id === signerId) {
  ...
  // Fallback: if no signerId provided or not found, mark the first unsigned signer
  if (!signerFound) {
    for (var k = 0; k < signers.length; k++) {
      if (!signers[k].hasSigned) {
        signers[k].hasSigned = true;
        signers[k].signedAt = new Date().toISOString();
        break;
      }
    }
  }
  ...
  var allSigned = true;
  for (var m = 0; m < signers.length; m++) {
    if (!signers[m].hasSigned) { allSigned = false; break; }
  }

  doc.status = allSigned ? 'COMPLETED' : 'IN_PROGRESS';
